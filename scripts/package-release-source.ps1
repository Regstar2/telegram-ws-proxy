...
$jlatexCommit = (& git -C $telegram ls-tree HEAD TMessagesProj/lib/jlatexmath).Trim().Split()[2]
if ($LASTEXITCODE -ne 0 -or $jlatexCommit -notmatch '^[0-9a-f]{40}$overlayArchive = Join-Path $output ("Telegram-WSP-overlay-source-{0}.zip" -f $upstream.telegramVersion)
